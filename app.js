...
class ColumnNode {
    constructor(name, type, allowNull, isPrimaryKey, isForeignKey, foreignKeyReference) {
        this.name = name;
        this.type = type;
...
class BST {
    ...
    _insertRec(root, node) {
        if (!root) return node;
        if (this.compareFn(node, root) < 0) root.left = this._insertRec(root.left, node);
        else if (this.compareFn(node, root) > 0) root.right = this._insertRec(root.right, node);
        return root;
    }
    ...
    _searchRec(root, target) {
        ...
        if (this.compareFn(target, root) < 0) return this._searchRec(root.left, target);
...
function addColumn(tableName) {
    ...
    table.columns.insert(new ColumnNode(columnName, columnType, allowNull, isPrimaryKey, isForeignKey, foreignKeyReference));
    renderTables();
}
...
function renderTables() {
    ...
    bst.inOrder(bst.root, node => {
        ...
        node.columns.inOrder(node.columns.root, col => {
            columnsHtml += `<li>${col.name} (${col.type}) - NULL: ${col.allowNull} | PK: ${col.isPrimaryKey} | FK: ${col.isForeignKey} ${col.foreignKeyReference ? `(${col.foreignKeyReference})` : ''}</li>`;
        });
        let recordsHtml = "";
        node.records.inOrder(node.records.root, record => {
            let recordData = Object.entries(record.data).map(([key, value]) => `${key}: ${value}`).join(", ");
